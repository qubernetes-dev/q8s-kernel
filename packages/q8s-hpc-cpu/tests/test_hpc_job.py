...
from q8s_hpc.hpc_job import (
    HPCJobTemplatePlugin,
    _parse_config_line,
    load_hpc_config,
...
class TestHPCJobTemplatePlugin(unittest.TestCase):

    @patch("q8s_hpc.hpc_job.load_hpc_config")
    @patch("q8s_hpc.hpc_job.client.V1Container")
    @patch("q8s_hpc.hpc_job.client.V1PodTemplateSpec")
    @patch("q8s_hpc.hpc_job.client.V1PodSpec")
    @patch("q8s_hpc.hpc_job.client.V1ObjectMeta")
    @patch("q8s_hpc.hpc_job.client.V1VolumeMount")
    @patch("q8s_hpc.hpc_job.client.V1Volume")
    @patch("q8s_hpc.hpc_job.client.V1ConfigMapVolumeSource")
    @patch("q8s_hpc.hpc_job.client.V1LocalObjectReference")
    def test_makejob_hpc_cpu(
        self,
        ...
        mock_load_hpc_config.return_value = {
            "path": "/tmp/HpcConfig",
            "q8s_node": "lumi-node-1",
            "slurm_flags": ["--account=project_123", "--time=01:00:00"],
        }

        plugin = HPCJobTemplatePlugin()
        name = "test-job"
        ...
        self.assertEqual(
            ...
            {"kubernetes.io/hostname": "lumi-node-1"},
        )
    ...
    def test_get_base_image(self):
        ...
        self.assertEqual(plugin.get_base_image("3.12"), "python:3.12-slim")
